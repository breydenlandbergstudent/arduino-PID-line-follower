...
void readSensorValues() {
  ...
  // -- |ON - ON - ON| --   stop the line follower
    ...
    // -- OFF |ON| OFF -- 
    if(sensor == LINE_MIDDLE) {
      error = 0;
    }
    // -- |ON - ON| OFF --
    else if(sensor == LINE_LEFT_CLOSE) {
      error = 2;
    }
    // -- |ON| OFF - OFF --
    else if(sensor == LINE_LEFT_FAR) {
      error = 4;
    }
    // -- OFF |ON - ON| --
    else if(sensor == LINE_RIGHT_CLOSE) {
      error = -2;
    }
    // -- OFF - OFF |ON| --
    else if(sensor == LINE_RIGHT_FAR) {
      error = -4;
    }
  }
}
